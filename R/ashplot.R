...
ash <- function(x, binwidth=NULL, adjust=NULL) {
  if (is.null(adjust)) adjust <- 1
  if (is.null(binwidth)) binwidth <- diff(range(x)) / (10 / adjust)
  left <- x - binwidth
  right <- x + binwidth
  ...
  ...) 
{
  ...
  if (sum(!is.na(x)) < 1) 
    ...
    h <- ash(x, binwidth = darg$width, adjust = darg$adjust)
    ...
    for (i in seq_along(vals)) {
      ...
      if (sum(id, na.rm = TRUE) > 1) {
        h <- do.call(ash, c(list(x = x[id], binwidth = darg$width, darg$adjust))) 
        xl <- c(xl, h$x)
        yl <- c(yl, h$y)
    ...
    identifier = "ash") 
{
  ...
  if (!is.null(groups)) {
    ...
    ash <- ash(x, darg$width, adjust=darg$adjust)    
    if (sum(!is.na(x)) > 1) {
