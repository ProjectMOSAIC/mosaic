...
inputPanel(
  selectInput("bin_width", label = "bin width:",
              choices = c(0.1, 0.2, 0.25, 0.5, 1.0, 1.5), selected = 0.25),
...
renderPlot({
  histogram( ~ eruptions, data = faithful, 
             ...
             xlab = paste0("Duration (minutes) [", input$bw_adjust, "]"), 
             main = "Geyser eruption duration",
             panel = function(x, ...) {
               panel.xhistogram(x, ...)
               panel.densityplot(x, n = 500, darg = list(adjust = input$bw_adjust), ...)
             })
})
```
