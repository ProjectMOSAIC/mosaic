...
if (FALSE) {
  bootstrap <- do(50) * diffmean(age ~ sex, data = HELPrct)
  bootstrap2 <- do(500) * mean( resample(1:10) )
  result1 <- confint(bootstrap)
  result2 <- confint(bootstrap, method = "stderr")
  result3 <- confint(bootstrap, method = "se", df = nrow(HELPrct) - 1)
  result4 <- confint(bootstrap, margin.of.error = FALSE)
  result5 <- confint(bootstrap, margin.of.error = TRUE, level = 0.99, method = c("boot", "se", "perc"))
  result6 <- confint(bootstrap2)
  save(bootstrap, bootstrap2, result1, result2, result3, result4, result5, result6, file = "tests/testthat/bootstrap.Rda")
}
...
getData <- function() {
  ...
  lazy = substitute(diffmean(age ~ sex, data = resample(HELPrct))), culler = function (object,...)
...
getDataAlternate <- function() {
  structure(list(mean = c(4.9, 5.2, 6, 7.3, 6.1, 6.7, 7.1, 5.7,
    ...
    row.names = c(NA, -500L), lazy = substitute(mean(resample(1:10))),
