...
The goal of this document is to demonstrate how to calculate the quantities described in Chapter 2: Inference Using \emph{t}-Distributions using R.
...
A total of \Sexpr{nrow(sparrow)} subjects are included in the data: \Sexpr{nrow(subset(sparrow, STATUS=="SURVIVED"))} are adult male sparrows that survived and \Sexpr{nrow(subset(sparrow, STATUS=="PERISHED"))} that perished. As showed in Display 2.1 (emph{Sleuth}, p.29).
...
Based on this information, we can construct a 95\% confidence interval. (Calculation on p.41, Display 2.9)
<<>>=
...
Or we can get the results of ``Summary of Statistical Findings" (p.29) by using the following code: 
<<>>=
...
This matches the results on p.30, Display 2.2.
...
<<>>=
t.test(schizo[, "UNAFFECT"], schizo[, "AFFECTED"], paired=TRUE)
@
