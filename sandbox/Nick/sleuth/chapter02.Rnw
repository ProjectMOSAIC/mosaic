...
The goal of this document is to demonstate how to calculate the quantities described in Chapter 2: Inference Using \emph{t}-Distributions using RStudio.
...
<<>>=
sparrow=read.csv("http://www.math.smith.edu/~nhorton/sleuth/case0201.csv")
summary(sparrow)
fav=favstats(HUMERUS ~ STATUS, data=sparrow); fav
@

A total of \Sexpr{nrow(sparrow)} subjects are included in the data: \Sexpr{nrow(subset(sparrow, STATUS=="SURVIVED"))} are adult male sparrows that survived and \Sexpr{nrow(subset(sparrow, STATUS=="PERISHED"))} perished. As showed in Display 2.1 (page 29 of \emph{Sleuth}).

<<>>=
bwplot(STATUS ~ HUMERUS, data=sparrow)
@

<<>>=
densityplot(~ HUMERUS, groups=STATUS, auto.key=TRUE, data=sparrow)
@
...
First, we calculate the pooled SD and the standard error between these two different sample average. (Calculation on p.40, Display 2.8)
<<>>=
# Calculate Pooled SD
...
Based on the information we got, we can construct 95\% confidence interval. (Calculation on p.41, Display 2.9)
<<>>=
...
Now we want to calculate the t-statistic and p-value. (Calculation on p.44, Display 2.10)
<<>>=
...
So the one-sided p-value is \Sexpr{round(onepval, 2)} and the two-sided p-value is \Sexpr{round(twopval, 2)}

Or we can get the results of "Summary of Statistical Findings" (on \emph{Sleuth} p.29) by using following code. 
<<>>=
t.test(HUMERUS ~ STATUS, var.equal=TRUE, data=sparrow)
confint(lm(HUMERUS ~ STATUS, data=sparrow))
@

\section{Anatomical Abnormalities Associated with Schizophrenia}

Is the area of brain related to schizophrenia? That's the question being addressed by case study 2.2 in the \emph{Sleuth}.

\subsection{Statistical summary and graphical display}
We begin by reading the data and summarizing the variables.

<<>>=
schizo=read.csv("http://www.math.smith.edu/~nhorton/sleuth/case0202.csv")
summary(schizo)
@

A total of \Sexpr{nrow(schizo)} subjects are included in the data. There are \Sexpr{nrow(schizo[ "AFFECTED"])} pairs of twins; one of the twins has schizophrenia, and the other does not. So there are \Sexpr{nrow(schizo["AFFECTED"])} affected subjects and \Sexpr{nrow(schizo["UNAFFECT"])} unaffected subjects.

The difference in area of left hippocampus of these pairs of twins is:
<<>>=
DIFF=schizo[, "UNAFFECT"]-schizo[, "AFFECTED"]
favstats(DIFF)
@

As showed in Display 2.2 (page 30 of \emph{Sleuth}).
...
Or we can get the results displayed on \emph{Sleuth} p.31 by conducting a paired t-test.

<<>>=
summary(lm(UNAFFECT ~ AFFECTED, data=schizo))
@
