...
A total of \Sexpr{nrow(sparrow)} subjects are included in the data: \Sexpr{nrow(subset(sparrow, STATUS=="SURVIVED"))} are adult male sparrows that perished and \Sexpr{nrow(subset(sparrow, STATUS=="PERISHED"))}. As showed in Display 2.1 (\emph{Sleuth}).
...
First, we calculate the pooled SD and the standard error between these two different sample average. (Calculation on p.40)
<<>>=
# Calculate Pooled SD
...
Based on the information we got, we can construct 95\% confidence interval. (Calculation on p.41)
<<>>=
...
Now we want to calculate the t-statistic and p-value. (Calculation on p.44)
<<>>=
...
A total of \Sexpr{nrow(schizo)} subjects are included in the data. they are \Sexpr{nrow(schizo[, "AFFECTED"])} pairs of twins; one of the twins is schizophrenia, whith the other one is not. So there are \Sexpr{nrow(schizo[, "AFFECTED"])} affected subjects and \Sexpr{nrow(schizo["UNAFFECT"])} unaffected subjects.
...
As showed in Display 2.2 (\emph{Sleuth}).
