...
The goal of this document is to demonstate how to calculate the quantities described in Chapter 5: Comparisons Among Several Samples using RStudio.
...
There is a strongly statistically significant difference between the diets (\Sexpr{print.pval(anova(lm(LIFETIME ~ DIET, data=mice))["Pr(>F)"])}, p.115).
...
<<>>=
aov1 = anova(lm(PERCENT ~ JUDGE, data=spock)); aov1
@
...
@
Recall that the book calculates the extra sum of squares as (2,190.90 - 1864.45)/(44-39)) / (1864.45 / 39) = 1.37, with df 5 and 39.  P(F $\textgreater$ 1.366) = 0.26 (pg 130).  Below are the calculations for the numbers found on page 128. 
...
<<>>=
model.tables(aov(lm(PERCENT ~ as.factor(TWOJUDGE), data=spock)))
@

There are some other ways to compare whether the other judges differ from Spock's judge in their female composition?

<<>>=
# test all of the other judges vs. Spock's judge using a contrast
fit.contrast(lm(PERCENT ~ JUDGE, data=spock), "JUDGE", c(1, 1, 1, 1, 1, 1, -6), conf.int=0.95)

# calculate the 95% confidence interval for Dr. Spock's jury female composition?
estimable(lm(PERCENT ~ JUDGE, data=spock), c(1,0,0,0,0,0,1), conf.int=0.95)
@

\subsection{Kruskal-Wallis Nonparametric Analysis of Variance}
