...
The goal of this document is to demonstrate how to calculate the quantities described in Chapter 5: Comparisons Among Several Samples using R.
...
There were a total of \Sexpr{nrow(mice)} female mice. These mice were randomly assigned to one of \Sexpr{length(unique(mice[, "DIET"]))} diets.  Their lifetimes were then recorded, as shown in Display 5.2 (page 115 of the \emph{Sleuth}).
...
The reference group is \emph{lopro}, followed by \emph{N/N85, N/R40, N/R50, NP, R/R50}.
...
@
There were a total of \Sexpr{nrow(spock)} venires. They compared Spock's judge with \Sexpr{length(unique(subset(spock, JUDGE!="SPOCK'S")[, "JUDGE"]))} other judges.  The precent of women within each venire was recorded as shown in Display 5.4 (page 117 of the \emph{Sleuth}).
...
First we fit the one way analysis of variance (ANOVA) model, with all of the groups. These results are summarized on page 118.
<<>>=
...
other judge. First we create a 2 level version of the grouping variable.
...
@
Recall that the book calculates the extra sum of squares as (2,190.90 - 1864.45)/(44-39)) / (1864.45 / 39) = 1.37, with df 5 and 39.  P(F $\textgreater$ 1.366) = 0.26 (p.130).  Below are the calculations for the results found on page 128. 
