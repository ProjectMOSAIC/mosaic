...
To verify the interval for the predicted values on page 279, we used the following code:
<<>>=
...
<<>>=
pred = predict(lm1, se.fit=TRUE, newdata=data.frame(newtype=c("non-echolocating birds", "non-echolocating birds"), logmass=c(log(100), log(400))))
...
pred2 = predict(lm1, se.fit=TRUE, newdata=data.frame(newtype=c("non-echolocating bats", "non-echolocating bats"), logmass=c(log(100), log(400))))
pred3 = predict(lm1, se.fit=TRUE, newdata=data.frame(newtype=c("echolocating bats", "echolocating bats"), logmass=c(log(100), log(400))))
