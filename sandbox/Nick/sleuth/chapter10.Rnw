...
The goal of this document is to demonstrate how to calculate the quantities described in Chapter 10: Inferential Tools for Multiple Regression using R.
...
There we a total of \Sexpr{nrow(galileo)} trials of Galileo's experiment.  For each trial, he recorded the initial height and then measured the horizontal distance as shown in Display 10.1 (p.268).

We can start to explore this relationship by creating a scatterplot of Galileo's horizontal distances versus initial heights.  The following graph is akin to Display 10.2 (p.269).
<<>>=
xyplot(DISTANCE ~ HEIGHT, data=galileo)
@

\subsection{Models}
The first model that we created is a cubic model as interpreted on page 269 and summarized in Display 10.13 (p.286).
<<>>=
lm3 = lm(DISTANCE ~ HEIGHT + I(HEIGHT^2) + I(HEIGHT^3), data=galileo); summary(lm3)
@

We next decrease \emph{height} by one degree to obtain a quadratic model as interpreted on page 269 and summarized in Display 10.7 (p.277). Notice that this model is used for most of the following results. 

<<>>=
lm2 = lm(DISTANCE ~ HEIGHT + I(HEIGHT^2), data=galileo); summary(lm2)
@

The following figure presents the predicted values from the quadratic model and the original data points akin to Display 10.2 (p.269).
...
To obtain the expected values of \^{$\mu$}$\{distance|height = 0\}$ and \^{$\mu$}$\{distance|height = 250\}$, we used the {\tt predict} command with the quadratic model as shown in Display 10.7 (p.277).
<<>>=
...
We can next observe the pattern between log(\emph{energy expenditure(W)}) as a function of log(\emph{body mass(g)}) for each group with a scatterplot. The following figure is akin to Display 10.4 (p.271).
...
We first evaluate a multiple regression model for log(\emph{energy expenditure(W)}) given type of species and log(\emph{body mass(g)}) as defined on page 272. 
<<>>=
lm1 = lm(logenergy ~ logmass + TYPE, data=echo); summary(lm1)
@
Notice that in R the reference group for {\tt TYPE} is not the same as in the book, therefore we can use {\tt relevel} to create a new variable which allows us to specify a reference group.  With this new {\tt TYPE} variable we can re-fit our model and the resulting table is similar to Display 10.6 (p.273).
<<>>=
...
To construct the confidence bands discussed on page 277 and shown in Display 10.9 (p.279) we used the following code: 
<<>>=
...
@
XX The confidence interval is not the same as in display 10.9 (7.9-12.0)XX

Next we can assess the model by evaluating the extra sums of squares $F$-test for testing the equality of intercepts in the parallel regression lines model as shown in Display 10.10 (p. 282).
<<>>=
lm2 = lm(logenergy ~ logmass, data=echo)
anova(lm2, lm1)
@

We can also compare the full model with interaction terms and the reduced model (without interaction terms) with the extra sum of squares $F$-test as described in Display 10.12 (p.284). 
<<>>=
