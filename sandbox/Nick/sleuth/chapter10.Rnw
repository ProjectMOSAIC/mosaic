...
To obtain the expected values of $\hat{\mu}\left(\mathrm{distance}|\mathrm{height} = 0 \right)$ and \hat{\mu}\left(\mathrm{distance}|\mathrm{height} = 250\}$, we used the {\tt predict()} command with the quadratic model as shown in Display 10.7 (page 277).
<<>>=
...
pred.fit = pred$fit[1]; predfit
pred.se = pred$se.fit[1]; predse
...
@
XX There are more numbers in Display 10.9 
