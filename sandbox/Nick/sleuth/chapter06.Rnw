...
This work leverages initiatives undertaken by Project MOSAIC (\url{http://www.mosaic-web.org}), an NSF-funded effort to improve the teaching of statistics, calculus, science and computing in the undergraduate curriculum. In particular, we utilize the mosaic package, which was written to simplify the use of R for introductory statistics courses. To use a package within R, it must be installed (one time), and loaded (each session). The package can be installed using the following command:
...
HSD.test(aov(lm(SCORE ~ HANDICAP, data=disability)), "HANDICAP")   # Tukey-Kramer
...
A total of \Sexpr{nrow(fish)} female fish were involved in this experiment (p.150).
...
XX RESPONSE: Isn't this just a test of proportions for this subgroup?  XX
