...
options(digits=3)
@
The goal of this document is to demonstrate how to calculate the quantities described in Chapter 6: Linear Combinations and Multiple Comparisons of Means using R.
...
A total of \Sexpr{nrow(disability)} undergradate students from a U.S. university were randomly assigned to view the tapes, \Sexpr{nrow(subset(disability, HANDICAP=="NONE"))} to each tape. The five kinds of tapes are: NONE, AMPUTEE, CRUTCHES, HEARING and WHEELCHAIR. After reviewing the tape, each subject rated the qualifications of the application on 0-10 scale. Among the five hadicap conditions, the CRUTCHES group gave the highest mean score, while the HEARING group gave the lowest mean score. This is summarized on page 150 and in Display 6.1 of the \emph{Sleuth}. 
...
bwplot(HANDICAP ~ SCORE, data=disability)
@

<<fig.height=8, fig.width=8>>=
densityplot(~SCORE, groups=HANDICAP, auto.key=TRUE, data=disability)
@
...
The p-value is \Sexpr{print.pval(anova(lm(SCORE ~ HANDICAP, data=disability))["HANDICAP","Pr(>F)"])}, which shows the evidence that subjects rate qualifications differently according to handicap status.
...
The Tukey-Kramer test is the most approporiate approach for these data.
...
<<echo=FALSE>>=
tuk = TukeyHSD(aov(lm(SCORE ~ HANDICAP, data=disability)), "HANDICAP", ordered=TRUE, c(0,1,-1,0,0), conf.level=0.95)
@

Based on the Tukey-Kramer procedure, the difference is estimated to be \Sexpr{round(tuk$HANDICAP["CRUTCHES-HEARING", "diff"], 2)} points higher for the CRUTCHES tapes, with a 95\% confidence from \Sexpr{round(tuk$HANDICAP["CRUTCHES-HEARING", "lwr"], 2)} to \Sexpr{round(tuk$HANDICAP["CRUTCHES-HEARING", "upr"], 2)}.
...
The t-value=\Sexpr{round(fit.contrast(lm(SCORE ~ HANDICAP, data=disability), "HANDICAP", c(-1, 1, -1, 0, 1), conf.int=0.95)[, "t value"], 2)} supports a difference between the average scores given to the WHEELCHAIR and CRUTCHES handicaps and the average scores given to the AMPUTEE and HEARING handicaps.
...
Other multiple comparison procedures could also be implemented. The following shows the calculation on p.164
<<>>=
library(agricolae)
...
Was Charles Darwin right that sexual selection is by females?  This is the question addressed in case study 6.2 in the \emph{Sleuth}.  The goal of this document is to demonstrate how to calculate the quantities described in chapter 6 (linear combinations and multiple comparisons) using R.
...
<<fig.height=8, fig.width=8>>=
bwplot(PAIR ~ PROPORTION, data=fish)
@

<<fig.height=8, fig.width=8>>=
densityplot(~PROPORTION, groups=PAIR, auto.key=TRUE, data=fish)
@
...
The p-value is \Sexpr{print.pval(anova(lm(PROPORTION ~ PAIR, data=fish))["PAIR","Pr(>F)"])}, which doesn't provide much evidence that the mean percentage of time with the yellow-sword male differed from one male pair to another.
...
We can calculate the values on page 152 and Display 6.5 on page 158 by using contrasts.
<<>>=
library(gmodels)
lc=fit.contrast(lm(PROPORTION ~ PAIR, data=fish), "PAIR", c(5, -3, 1, 3, -9, 3), conf.int=0.95); lc
t=round(lc[, "t value"], 2); t
pt(t, 78, lower.tail=TRUE)
@

The t-value is \Sexpr{round(fit.contrast(lm(PROPORTION ~ PAIR, data=fish), "PAIR", c(5, -3, 1, 3, -9, 3), conf.int=0.95)[, "t value"], 2)} and the one-sided p-value is \Sexpr{round(pt(t, 78, lower.tail=TRUE), 2)}.

XX We didn't find way to calculate the values shown in the following sentences: The mean percentage of time spent with the yellow-sword male is estimated to be 62.4\%. A 95\% confidence interval for the mean percentage is 59.0\% to 65.8\%. This is convincing evidence (one-sided $p-value<0.0001$ that the mean percentage exceeds the lack-of-perference value (50\%). XX
