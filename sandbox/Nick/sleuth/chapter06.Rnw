...
The p-value is \Sexpr{pval(anova(lm(PROPORTION ~ PAIR, data=fish)))}, which doesn't provide evidence that the mean percentage of time with the yellow-sword male differed from one male pair to another.
