...
<<>>=
summary(seaweed)
favstats(logitcover~TREAT, data=seaweed)
...
<<fig.height=8, fig.width=8>>=
plot(aov(COVER ~ BLOCK*TREAT, data=seaweed), which=1)
@
...
@
%XX is there a way to order the blocks so it looks more like Display 13.9?
...
This model has an $R^2$ of \Sexpr{round(summary(lm(logitcover ~ BLOCK*TREAT, data=seaweed))$r.squared*100, 2)}$\%$, an adjusted $R^2$ of \Sexpr{round(summary(lm(logitcover ~ BLOCK*TREAT, data=seaweed))$adj.r.squared*100, 2)}$\%$, and an estimated SD of \Sexpr{round(summary(lm(logitcover ~ BLOCK*TREAT, data=seaweed))$sigma, 4)}. Notice that the interaction term has a large $p$-value, \Sexpr{round(anova(lm(logitcover ~ BLOCK*TREAT, data=seaweed))["BLOCK:TREAT", "Pr(>F)"], 4)}, suggesting that the data may be more consistent with an additive model.  
...
<<>>=
model.tables(aov(lm(logitcover ~ BLOCK*TREAT, data=seaweed)), type="effects")
@

For the means we changed the {\tt type} attribute to {\tt "means"}:
<<>>=
model.tables(aov(lm(logitcover ~ BLOCK*TREAT, data=seaweed)), type="means")
@
...
<<>>=
obs = summary(lm(SCORE ~ COMPANY+TREAT, data=pygmalion))$coefficients["TREATPYGMALION", "t value"]
nulldist = do(15000) * summary(lm(SCORE ~ shuffle(COMPANY)+shuffle(TREAT), data=pygmalion))$coefficients["shuffle(TREAT)PYGMALION", "t value"]
xhistogram(~ result, groups=result >= obs, v=obs, data=nulldist) # akin to Display 13.20 page 398
