...
The goal of this document is to demonstrate how to calculate the quantities described in Chapter 13: The Analysis of Variance for Two-Way Classifications using R.
...
This model has an R-squared of \Sexpr{round(summary(lm(logitcover ~ BLOCK*TREAT, data=seaweed))$r.squared*100, 2)}$\%$, an adj. R-squared of \Sexpr{round(summary(lm(logitcover ~ BLOCK*TREAT, data=seaweed))$adj.r.squared*100, 2)}$\%$, and an estimated SD of \Sexpr{round(summary(lm(logitcover ~ BLOCK*TREAT, data=seaweed))$sigma, 4)}.
...
Next we will assess the normality assumption for the additive model XX WHY DO WE CHANGE MODELS? XX
...
Now we can assess equality of variance for the additive model which, from the figure, seems to be somewhat problematic.
<<fig.height=8, fig.width=8>>=
...
To answer specific questions of interest we can use linear combinations.  The following is interpreted on pages 389-390 and are found in Display 13.13.  For this model the reference group is \emph{control} followed by \emph{f, fF, L, Lf, LfF}.   Note the error in the sign of the $t$-statistic.
<<>>=
library(gmodels)
...
We can then use multiple linear regression models for the additive and nonadditive models and compare them using the Two-way ANOVA.  The following is similar to Display 13.16 (p.394).  
<<>>=
