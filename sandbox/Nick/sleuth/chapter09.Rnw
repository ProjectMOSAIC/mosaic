...
light$day24 = relevel(as.factor(ifelse(light$TIME==1, "late", "early")), 
                      ref="late")
@
...
<<>>=
xyplot(FLOWERS ~ INTENS, groups=day24, type=c("p", "r", "smooth"), 
       data=light, auto.key=TRUE, 
...
mammals$loggest = log(mammals$GESTATION); 
...
<<>>=
lm1 = lm(logbrain ~ logbody+loggest+loglitter, data=mammals)
summary(lm1)
@
