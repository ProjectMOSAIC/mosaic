...
The goal of this document is to demonstrate how to calculate the quantities described in Chapter 9: Multiple Regression using R.
...
A total of \Sexpr{nrow(light)} meadowfoam plants were included in this data.  There were \Sexpr{length(unique(light[,"TIME"]))*length(unique(light[, "INTENS"]))} treatment groups -  \Sexpr{length(unique(light[, "INTENS"]))} light intensities at each of the  \Sexpr{length(unique(light[,"TIME"]))} timing levels. As shown in Display 9.2 (page 237 of the \emph{Sleuth}).
...
<<>>=
xyplot(FLOWERS ~ INTENS, groups=day24, type=c("p", "r", "smooth"), data=light, auto.key=TRUE, xlab="light intensity (mu mol/m^2/sec)", ylab="average number of flowers")
@


\subsection{Multiple linear regression model}

We then can fit a multiple linear regression model that specifies parallel regression lines for the mean  number of flowers as a function of light intensity as interpreted on page 237.
...
A total of \Sexpr{nrow(mammals)} mammals were included in this data.  The average values of brain weight, body weight, gestation length, and litter size for each of the species were calculated and presented in Display 9.2 (page 237 of the \emph{Sleuth}).
...
xyplot(BRAIN ~ jitter(LITTER) | weightcut, 
       scales=list(y=list(log=TRUE), x=list(log=TRUE)), type=c("p", "r"), data=mammals)
@
