...
The goal of this document is to demonstrate how to calculate the quantities described in Chapter 4: The Rank-Sum Test using R.
...
A total of \Sexpr{nrow(oring)} subjects are included in the data: \Sexpr{nrow(subset(oring, LAUNCH=="COOL"))} O-ring incidents when temperature was cold and \Sexpr{nrow(subset(oring, LAUNCH=="WARM"))} O-ring incidents when temperature was warm. As shown in Display 4.1 , p. 86.
...
To replicate the permutation test performed on page 96 we used the following code.

First, we calculate the $t$-statistic of the observed outcome.
...
Alternatively, we can approximate the $p$-value using the difference of means and simulating a null distribution. 
...
We conduct Wilcoxon rank-sum test to calculate the results displayed on p.88.
