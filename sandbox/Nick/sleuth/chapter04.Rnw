...
<<>>=
# t.test for (1, 1, 2, 3)
t.test(oring[c(1,2,4,24),]$INCIDENTS, oring[-c(1,2,4,24),]$INCIDENTS, var.equal=TRUE)
# t.test for (0, 1, 2, 3)
t.test(oring[c(1,4,5,24),]$INCIDENTS, oring[-c(1,4,5,24),]$INCIDENTS, var.equal=TRUE)
@

The $t$-statistic for (1, 1, 2, 3) is \Sexpr{t.test(oring[c(1,2,4,24),]$INCIDENTS, oring[-c(1,2,4,24),]$INCIDENTS, var.equal=TRUE)$statistic} and the $t$-statistic for (1, 1, 2, 3) is \Sexpr{t.test(oring[c(1,4,5,24),]$INCIDENTS, oring[-c(1,4,5,24),]$INCIDENTS, var.equal=TRUE)$statistic}

XX Ruobing: I can only get the t-statistics shown on the book by turning the option var.equal=TRUE. If it's OK, please not delete it. XX
...
XX Why are the p-values so different? Strict rounding? XX  
XX RESPONSE: I changed to less than or equal: does that help? XX
XX Ruobing: Not really. The p-value calculated using book's method is around 0.01, but the one calculated using computer simulation is around 0.1. XX
...
XX Ruobing: Yes, there are, but I'm still on my way to figure out how to present those displays. This case is about rank-sum test and there is a table listing all observations and explaining the procedures of assigning orders and ranks. I haven't figured out how to show the calculation of p-value in the document. XX
...
XX The book says that the 95\% confidence interval should be (58, 159). However, what I got from wilcox.test is a little bit wilder, which is (57, 160); if I turn correct=FALSE, I'll get a narrower result, which is (59, 158). How can I adjust this? XX

<<>>=
rank(teaching$TIME, ties.method="average")
@
