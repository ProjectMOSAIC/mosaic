...
A total of \Sexpr{nrow(rain)} subjects were included in this data: \Sexpr{nrow(subset(rain, TREATMENT=="SEEDED"))} seeded days  and \Sexpr{nrow(subset(rain, TREATMENT=="UNSEEDED"))} 
...
The two-sided $p$-value is \Sexpr{print.pval(pval(t.test(lograin ~ TREATMENT, data=rain)))} and the 95\% confidence interval is between \Sexpr{round(ttestlog$conf.int[1], 2)} and \Sexpr{round(ttestlog$conf.int[2], 2)}.
...
A total of \Sexpr{nrow(ao)} veterans were included in this data: \Sexpr{nrow(subset(ao, VETERAN=="VIETNAM"))} served in Vietnam during 1967 and 1968  and \Sexpr{nrow(subset(ao, VETERAN=="OTHER"))} served in US or Germany during 1965 and 1971.
...
<<>>=
t.test(DIOXIN ~ VETERAN, var.equal=TRUE, alternative="less", data=ao)
t.test(DIOXIN ~ VETERAN, var.equal=TRUE, data=ao)$conf.int
@

So the one-sided $p$-value from a two-sample $t$-test is \Sexpr{pval(t.test(DIOXIN ~ VETERAN, var.equal=TRUE, alternative="less", data=ao))}. The 95\% confidence interval is (\Sexpr{round(t.test(DIOXIN ~ VETERAN, var.equal=TRUE, data=ao)$conf.int[1], 2)}, \Sexpr{round(t.test(DIOXIN ~ VETERAN, var.equal=TRUE, data=ao)$conf.int[2], 2)}).
...
<<>>=
ao2 = ao[-c(646), ]
t.test(DIOXIN ~ VETERAN, alternative="less", data=ao2)
@
...
ao3 = ao[-c(645, 646), ]
dim(ao3)
t.test(DIOXIN ~ VETERAN, alternative="less", data=ao3)
@
