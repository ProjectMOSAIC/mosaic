...
For each of the pages within the CMU OLI Statistics modules which included R code, we have taken those commands along with the introductory material regarding the datasets and included them below as the \emph{Original} code.  We have then replicated this functionality in the section entitled \emph{Revised}.
...
YY Not sure what happened here.
...
YY Not sure what you mean by driver functions.
...
@
YY But I would avoid using with() if possible.
<<>>=
t.test( Birthweight$birthweight, conf.level=0.99 )
@
YY If we get t.test() working properly, this will become
<<eval=FALSE>>=
t.test( ~birthweight, data=Birthweight )
confint(t.test( ~birthweight, data=Birthweight, conf.level=.99 ))
@
...
YY The \function{stat} function extracts test statistics from htest objects.
...
YY R functions should usually not return a string.  That makes it hard to do downstream 
computations. Better is to return a list or a vector or a data frame or an object in some 
user-defined class with a custom print method.  


YY Also, adding named arguments to functions makes it clearer what is going on and also
...
\subsection{Re-revised}
The \pkg{TeachingDemos} package includes a \function{z.test()} function.
<<>>=
require(TeachingDemos) # for z.test(); rjp has a version as well
z.test(Pregnancy$length, sd=16, mean=266, alternative="less")
@
\section{Module 12 (page 216)}
...
z.test = function(x, sigma, mu0, alt="greater") {
  ...
  # even better would be to return an htest object
  # rjp has code to do this (it is an exercise in FASt) as does the TeachingDemos package
  c(mean=mu, n=n, z=z, "p-value"=p)  
}
z.test(EPA$mpg, 4.7, 20, "greater");
@

\subsection{Re-revised}
<<>>=
require(TeachingDemos)
z.test( EPA$mpg, 20, sd=4.7, alternative="greater" )
@
