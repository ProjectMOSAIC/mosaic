...
XX This is kind of a mess: can we revisit how to do this? XX
...
require(TeachingDemos)
z.test(na.omit(Cellphones$Verbal), mu=580, stdev=111, alternative="greater")
@
...
require(mosaic)
head(TV2)
histogram(~ time | gender, data=TV2)
densityplot(~ time, groups=gender, auto.key=TRUE, data=TV2)
@
...
tab
tally(~ low | smoke, margins = FALSE, format = "percent", data = LBW)
...
tab
tally(~ low | race, margins = FALSE, format = "percent", data = LBW)
xchisq.test(tab, correct=FALSE)
@
...
confint(lm(IQ ~ cry.count, data=Baby))
@
