...
library(mosaic)
head(Actor);
histogram(~ Age, nint=8, data=Actor)
@
...
head(Graduation);
summary(Graduation);
bwplot(College.A, College.B, College.C, College.D, data=Graduation);
@
...
<<eval=TRUE>>=
library(mosaic)
xyplot(weight ~ height, groups=gender, auto.key=TRUE, data=Height)
@
...
n = length(Guns$opinion); n
x = length(subset(Guns, opinion=="satisfied"), opinion); x
...
XX dataset changes enumerated here XX
