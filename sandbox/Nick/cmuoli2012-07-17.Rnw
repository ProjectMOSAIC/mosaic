...
favstats(Final ~ factor(Extra_Credit), data = Gradebook)
...
favstats(Final ~ factor(Class), data = Gradebook)
...
favstats(age ~ factor(low), data = LBW)
...
favstats(Premium ~ factor(Female), data = Autopremiums)
