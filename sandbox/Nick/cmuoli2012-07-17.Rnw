...
We have tried to minimize the changes to the datasets to keep backward compatibility, but for pedagogical reasons have revised several dataset names and recoded some variables (see Appendix for details of these manipulations). There is a name confict with the {\tt Animals} dataset within the MASS package; this should be renamed in the {\tt mosaic} package.
...
<<eval=TRUE>>=
library(mosaic)
tab = tally(~Cell, margins = FALSE, data = Cellphones)
tab
tally(~Cell, margins = FALSE, format = "percent", data = Cellphones)
pie(tab, labels = c("no", "yes"))
n = tally(~Cell, data = Cellphones)
prop.test(243, 310, 0.80, alternative = "less", correct = FALSE)
...
NOTE: We need to rename the {\tt Animals} dataset to prevent a name conflict with the MASS package.
