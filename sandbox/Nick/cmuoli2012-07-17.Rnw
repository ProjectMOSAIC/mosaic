...
\begin{itemize}
	...
	Furthermore, data set names never match names of variables they contain.
...
There is currently a name confict with the \dataframe{Animals} dataset within the MASS package;
this data set should eventually be renamed in the \pkg{mosaic} package.  
...
<<eval=TRUE>>=
require(mosaic)
require(vcd)  # vcd provides mosaic() as an alternative to mosaicplot()
tally(~ Nearsightedness | Light, data=Nightlight)
tally(~ Light | Nearsightedness, data=Nightlight)
...
XX This will be fixed when Nick renames the \dataframe{Animals} dataset. XX
<<eval=TRUE>>=
require(mosaic)
head(mosaic::Animals)
xyplot(gestation ~ longevity, groups = animal == "elephant", data=mosaic::Animals)
with(mosaic::Animals, cor(longevity, gestation))
with(subset(mosaic::Animals, animal != "elephant"), cor(longevity, gestation))
@
...
precision.  This is visible in the scatter plot, and even moreso in histograms.
...
<<eval=TRUE>>=
require(mosaic)
tally(~ Gender + WtFeel, margins=FALSE, data=Bodyimage)
tally(~ Gender + WtFeel, margins=FALSE, format = "percent", data=Bodyimage)
@
...
mosaicplot( ~ Sex + Handed, Population, main = "Population")
mosaicplot( ~ Sex + Handed, Population, main = "random sample")
@
...
@
<<>>=
mosaicplot(~Handed + Course, Population, main="Population" )
mosaicplot(~Handed + Course, Random_Sample, main="Sample")
@
...
# Codebook
# --------
# Hospt: The patient's hospital, represented by a code for each of the 5
# hospitals (1, 2, 3, 5, or 6)
#
# Treat: The treatment received by the patient (0=Lithium, 1=Imipramine, or 2=Placebo)
#
# Outcome: Whether or not a recurrence occurred during the patient's treatment
# (Recurrence or No Recurrence)
#
# Time: Either the time in days till the first recurrence, or if a recurrence
# did not occur, the length in days of the patient's participation in the
# study.
#
# AcuteT: The time in days that the patient was depressed prior to the study.
# Age: The age of the patient in years, when the patient entered the study.
...
tally(~ Treat + Outcome, margins=FALSE, data=Depression)
tally(~ Treat + Outcome, margins=FALSE, format = "percent", data=Depression)
@

\subsection{Q1 Re-Revised}
<<eval=TRUE>>=
require(mosaic)
Depression$treat = factor(Depression$Treat, labels=c('Lithium','Imipramine','Placebo'))
tally( ~ treat + Treat, Depression ) # check accuracy of recoding
# once the treatment is coded as a factor, we can use stripplot()
# the use of jitter makes the plot easier to read.
stripplot(AcuteT ~ treat, data=Depression, jitter=0.5)
@
...
\subsection{Q2 Re-Revised}
<<eval=TRUE>>=
require(mosaic)
stripplot(Time ~ treat, data=Depression, jitter=0.5)
favstats(Time ~ treat, data=Depression)
@
...
<<eval=FALSE>>=
require(mosaic)
# on our wish list for the mosaic package is the following syntax
t.test( ~ birthweight, Birthweight, conf.level=0.99)
@

Note: This is an example where data frame and variable have the same name.  This is 
confusing.
...
require(mosaic)
head(Guns)  
tally(~ opinion, data=Guns)
prop.test(~opinion, data=Guns)
# selecting the other level as the reference level:
prop.test(~opinion, data=Guns, success='satisfied')
@
...
\subsection{Revised}
The \pkg{TeachingDemos} package includes a \function{z.test()} function.  We could consider adding a dependency on this with the \pkg{mosaic} package.
...
<<eval=TRUE>>=
require(mosaic)
histogram(~Verbal, data = Cellphones)
favstats(~Verbal, data = Cellphones)
require(TeachingDemos)
z.test(Cellphones$Verbal, mu=580, stdev=111, alternative="greater")
@
...
<<eval=TRUE>>=
require(mosaic)
tally(~Cell, margins = FALSE, data = Cellphones)
tally(~Cell, margins = FALSE, format = "percent", data = Cellphones)
pie(tab, labels = c("no", "yes"))
prop.test(~ Cell, Cellphones, 0.80, alternative = "less", correct = FALSE, success="yes")
z=(0.783871-0.80)/sqrt(0.80*(1-0.80)/310); z
@
...
<<eval=TRUE>>=
require(mosaic)
histogram(~Sleep, data = Cellphones)
favstats(~Sleep, data = Cellphones)
with(Cellphones, t.test(Sleep, mu = 7, alternative = "two.sided", conf.level = 0.95))
@
...
require(mosaic)
head(TV1)
histogram(~ time, subset=gender=="Female", data=TV1, main="Female")
histogram(~ time, subset=gender=="Male", data=TV1, main="Male")
@
<<>>=
densityplot(~ time, groups=gender, data=TV1)
@

Note: On our wish list is a frequency polygon function.
...
# an alternative -- can be applied to model objects saved from lm()
anova(lm(cff~color,flicker));
@
...
anova(lm(cff ~ color, data=Flicker))
@
...
<<eval=TRUE>>=
require(mosaic)
bwplot(Final ~ factor(Extra_Credit), data=Gradebook)
favstats(Final ~ Extra_Credit, data=Gradebook)
t.test(Final ~ Extra_Credit, alternative = "less", data=Gradebook)
@
...
require(mosaic)
head(Gradebook)
histogram(~Diff_Mid, data=Gradebook)
favstats(~Diff_Mid, data=Gradebook)
with(Gradebook, t.test(Midterm1 - Midterm2, mu = 0, alternative="greater"))
@
...
<<eval=TRUE>>=
require(mosaic)
bwplot(Final ~ factor(Class), data=Gradebook)
favstats(Final ~ Class, data=Gradebook)
anova(lm(Final ~ factor(Class), data=Gradebook))
@
...
<<eval=TRUE>>=
require(mosaic)
tab = tally(~smoke+low, margins = FALSE, data = LBW)
tab
tally(~smoke+low, margins = FALSE, format = "percent", data = LBW)
xchisq.test(tab, correct = FALSE)
@
...
<<eval=TRUE>>=
require(mosaic)
tab = tally(~race + low, margins = FALSE, data = LBW)
tab
tally(~race + low, margins = FALSE, format = "percent", data = LBW)
xchisq.test(tab, correct=FALSE)
@
...
<<eval=TRUE>>=
require(mosaic)
bwplot(age ~ factor(low), data = LBW)
favstats(age ~ low, data = LBW)
t.test(age ~ low, data = LBW)
@
...
<<eval=TRUE>>=
require(mosaic)
xyplot(Premium ~ Experience, data=Autopremiums)
with(Autopremiums, cor(Experience, Premium))
summary(lm(Premium ~ Experience, data=Autopremiums))
@
...
<<eval=TRUE>>=
require(mosaic)
bwplot(Premium ~ factor(Female), data=Autopremiums)
favstats(Premium ~ Female, data = Autopremiums)
t.test(Premium ~ Female, alternative = "greater", data=Autopremiums)
@

\chapter{Appendix}

The following R code was used to process the CMU OLI datasets into the form used in the \pkg{mosaic} package on CRAN. This may be helpful in determining what changes are needed for compatibility purposes.

NOTE: We need to rename the \dataframe{Animals} dataset to prevent a name conflict with the MASS package.
