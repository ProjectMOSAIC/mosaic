...
the following commands also load the \pkg{mosaicData} package that contains a number 
of data sets and set the number of digits to display by default. 
...
quantiles <- qdata(c(.025, .975), result, data=trials); quantiles
@
...
histogram(~ Sleep, groups=(Sleep >= obs),  data=trials, width=0.4,
...
<<>>=
trials = do(1000)* pval( chisq.test( tally( ~ shuffle(homeless) + sex, 
                         data=HELPrct, margins=FALSE)))
@
\end{enumerate}
