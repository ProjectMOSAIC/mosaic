...
#' @param class an object classs
#' @param add a logical indicating where we are adding
#' @noRd
#'
...
#'
#' @param x a model (lm)
#' @returns nothing; used for the side effect of priting the model as text.
#' @noRd
display_model <- function(x) {
  call_str <- x$call |> format()
  cat(paste(c("Call: ", call_str), collapse = ""))
  cat("\n")
}
