...
reduce the number of commands (R functions) and code templates (general schemes that unify the usage of 
...
The key tasks we wanted our students to master included
  ...
  3. The construction, visualization, and interpretation of models  (linear regression, ANOVA, logistic regression, etc.); and 
...
and to perform inference procedures.  Because of this goal, 
our most important template makes use of a "formula interface"
modeled after \code{lm()} and the plotting functions in \CRANpkg{lattice} \citep{lattice}. 
...
mapped to the vertical and horizontal axes, and `mydata` is the data frame in 
which these variables are found.
...
data set, whihc records the number of live births
...
```
\noindent
Using the \pkg{mosaic} package, this same template can be used to create numerical summaries.
```{r}
...
Packages such as \CRANpkg{resample} citep{resample} 
