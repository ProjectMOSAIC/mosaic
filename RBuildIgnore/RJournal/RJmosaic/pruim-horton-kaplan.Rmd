...
is being communicated to R.  Consider the following alternatives.

```{r}
with(Births78, aggregate(births, FUN = mean, by = list(wday)))
with(Births78, tapply(births, wday, mean))
```
While these generate similar output to the `mosaic` aggregation functions, the syntax is
...
"impressive" than one-variable plots and less likely to be something students can as 
...
births on the two weekend days compared to the other five days of the week:
```{r}
xyplot(births ~ date, groups = wday, data = Births78, type = "l")
densityplot( ~ births, groups = wday, data = Births78, auto.key = list(columns = 3))
```
These plots reveal a subtlety in interpreting the `Whickham` data set:
```{r}
...
A number of functions make it simple to visualize random variables.  The `plotDist()` function creates
...
The `stats::t.test()` function allows for a formula interface for 2-sample t tests, but not for 1-sample tests.
Similarly, the usual `binom.test()` and `prop.test()` functions do not provide a formula interface at all and work only
...
\code{plotFun()}, and \code{plotModel()} make these tasks easier.
...
object. The `print()`, `plot()`, and `summary()` functions are examples of extractors that can be applied to many
...
confidence intervals for the coefficients of a regression model.
...
```
\noindent
Readers familiar with \pkg{lattice} will notice that the \pkg{mosaic} package
adds some additional arguments to the `histogram()` function.  Among these are 
...
computed.
We typically introduce percentile confidence intervals first (but note that these
can have poor performance for small sample sizes).
...
returns an object that is inscrutable and unusable for most beginners.
...
The main motivation for `plotPoints()` is the ability to use it to create additional layers on an existing plot
with the option `add = TRUE`, otherwise `xyplot()` would suffice.  

## Basic choropleth maps

The `mUSMap()` and `mWorldMap()` functions provide simple 
...
encourage its use along side \pkg{mosaic}.  Unfortunately, there are several
...
Using R is not without some challenges.  The first challenge is to get all 
of the students
...
In our experience, the most commonly occurring struggles for students using \pkg{mosaic}
are
 ...
 1. General anxiety over typing commands.
    ...
    ``Less Volume, More Creativity" approach helps with this, by reducing the cognitive burden, 
    but it remains
    ...
    and comfortably recover from inevitable typing errors.  
    Even if a class does not typically meet in a computer laboratory
    or take advantage of student laptops, it can be useful to arrange some sessions early in 
 ...
 3. Difficulty in setting up the R environment
    ...
    This is all but eliminated when using an RStudio server, but in situations where instructors prefer
...
```
(again, this can also be done via the Packages tab in RStudio) will take care of the rest.  But occasionally
...
Similarly, using `do()` comes at a price, although here the increased computation
...
even on a laptop with a single quad-core processor, the speed-up is noticeable.
...
The instructor's temptation is often to do too much, forgetting the cognitive burden this can place on students.
...
DUE 0920350 (Project MOSAIC).  We thank Xiaofei (Susan) Wang and the reviewers for helpful 
comments and all of the users of the \pkg{mosaic} package who have provided feedback
