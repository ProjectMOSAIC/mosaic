...
using \CRANpkg{lattice} functions.
```{r}
...
XX consider citing Minimal R and Less Volume, More Creativity vignettes in or around here?  XX
...
produced via `plot()` but two primary differences: the user may select to use either \CRANpkg{lattice} or \CRANpkg{ggplot2} graphics insteado of base graphics, and an additional plot type is provided to visualize the 
...
mplot(mod, system = "ggplot2", which = 1:7)
```
...
\noindent
The menu allows the user to choose either \CRANpkg{lattice} or \CRANpkg{ggplot2}
...
Table~\ref{tab:otherstuff} lists some additional functions in the \CRANpkg{mosaic} package 
...
\\
\hline
\texttt{bargraph()}  & create bar graphs from raw data without tabulating first
\\
...
\\
\texttt{plotPoints()} &
an alternative to \texttt{xyplot()} that can also be used to create additional layers in
existing plots.
\\
\texttt{TukeyHSD()} & an extension to \texttt{stats::TukeyHSD()} that can take an lm object as input
and offers some additional plotting options.
\\
