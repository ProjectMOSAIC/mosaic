...
trellis.par.set(fontsize = list(text=9, points = 5))
set.seed(123)
```
...
the call the underlying functions eventually) and may be noticible slower in contexs where
...
The one-variable template can be obtained by removing the left-hand side from the formaula
...
to ask questions of the form "How does this depend on that (and some otehr things)?". 
...
such functions using a formaul interface, and use `plotFun()` to plot them.
```{r}
f <- makeFun(A *x^2 + B*x + C ~ x, A = 1, B = 1, C = 1)
f
f(2)
plotFun(f(x) ~ x, xlim = c(-2,2))
```
...
\noindent
For logisitic regression with factors, we need to adjust things slightly because
the model functions returns values between 0 and 1, but 2-level factors are coded 
as 1 and 2.
```{r}
...
(b) the resutling function returns values on the response scale by default, and (c) it 
backtransforms a few common transformations of the response variable, 
...
```
\noindent
Notice that `do()` (technicaly `cull_for_do()`) has been clever about what information 
...
To faciliate randomization and bootrapping, \CRANpkg{mosaic} extends `sample()` to operate
...
For example, we can use radomization in place of the two-sample t test to
...
or to allow for different confidence levels is a matter of some subtlty.  
See \cite{Hesterberg:2015}.

Each of these can be further automated using an extension to `conf.int()`.
```{r}
