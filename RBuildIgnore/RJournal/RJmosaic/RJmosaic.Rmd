...
```{r, include=FALSE}
require(mosaic)
knitr::opts_chunk$set(fig.width = 3, fig.height = 2, fig.show="hold", cache=TRUE)
options(digits = 3)
...
```
\noindent
and we have introduced that `tally()` function for counting categorical variables.
```{r}
...
clearly explaining the roles of the components of the variables for plotting, 
...
Users also have the option of changing this default for `na.rm` if they like.
...
on the important parts of these techniques (like where randomness enters in and how to
...
The statistical  punchline is that we want to compute the p-value for a binomial 
...
cdata(0.95, ~diffmean, data = Boot.dist2)
```
...
Each of these can be further automated using and extension to `conf.int()`.
```{r}
confint(Boot.dist2, method = c("percentile", "stderr"))
```

```{r}
Boot.dist3 <- do(1000) * favstats(age ~ sex, data = HELPrct)
```  
