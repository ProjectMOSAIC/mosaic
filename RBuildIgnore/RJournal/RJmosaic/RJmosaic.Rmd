...
bwplot(wday ~ births, data = Births78)
```
\noindent
...
```{r, fig.height = 5}
mplot(TukeyHSD(lm(births ~ wday, data = Births78)))
mplot(TukeyHSD(lm(births ~ wday, data = Births78)), system = "ggplot2") 
```
\noindent
...
An improvementon to the "t with bootstrap standard error" interval is the bootstrap-t interval.
...
```{r}
BootT <- do(1000) * favstats(age ~ sex, data = resample(HELPrct, groups = sex))
confint(BootT, method = "boot")
```
