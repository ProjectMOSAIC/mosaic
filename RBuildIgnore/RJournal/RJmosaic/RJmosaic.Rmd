...
abstract: >
  The mosaic package provides a simplified and systematic introduction to the core functionality related to descriptive statistics, graphical representations,  modeling, and simulation based inference required in first and second courses in  statistics. This introduction to the package describes some of the guiding principles  behind the design of the package and provides illustrative examples of several of the most  important functions it implements.  These can be combined to help students  ``think with data" using R in their early course work,  starting with simple, yet powerful, declarative commands.
...
available to them 
students need additional facility to express statistical computations 
(CITE NOLAN AND TEMPLE LANG 2010; Ridgway 2015 ISR; Horton,
Baumer, and Wickham 2015).
...
in a way that provides students with success and confidence.
...
```
```{r}
bwplot(length ~ sex,   data = KidsFeet);      bwplot(sex ~ length,   data = KidsFeet)
```
\noindent
With the \CRANpkg{mosaic} package attached (XX loaded? XX), the same template can be used to create 
numerical summaries.
```{r}
...
```
\noindent
We introduced a `tally()` function for counting categorical variables.
```{r}
...
XX Consider moving this benchmarking subsection to the discussion as a limitation?  XX
...
that the most interesting parts of statistics involve more than one variable (XX considing citing Wild CJ, Pfannkuch M, Regan M and Horton NJ.  Towards more accessible conceptions of statistical inference, Journal of the Royal Statistical Society: Series A (Statistics in Society), 2011; 174(part 2):247-295 XX), and 
...
to learn. XX consider citing Minimal R and Less Volume, More Creativity vignettes in or around here?  XX
...
The second solution is to change the default behavior of `na.rm`.  XX please review XX
...
# XX what about with(HELPmiss, base::mean(dayslink)))?
```
...
Resampling approaches have become increasingly important in statistical education (XX cite Hesterberg and Tintle papers in TAS 2015?  XX)
...
XX NH proposal: add mplot for regression diagnostics, coefficient plots, and dataset visualization (featuring "show expression")


XX NH proposal: for others include a brief mention as a table XX
...
XX NH votes no, but I would have no objections to a paragraph that mentions the Start R in Calculus resources and a pointer to the Macalester course. XX
