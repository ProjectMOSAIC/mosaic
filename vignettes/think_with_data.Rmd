...
output: 
  pdf_document: 
    ...
    keep_tex: yes
    includes:
      in_header: think_with_data_header.tex
---
...
[@NolanTempleLang:2010; @Ridgway:2015; @HortonBaumerWickham:2015]).
...
The \pkg{mosaic} [@mosaic] package originated in early attempts by 
...
presented as part of a roundtable discussion at the Joint Statistics Meetings [@Pruim:MinimalR:2011]
...
modeled after \code{lm()} and the plotting functions in \pkg{lattice} [@lattice]. 
...
using \pkg{lattice} functions.  Here we illustrate using the `Births78`
...
its use with another data set from \pkg{mosaicData} [@mosaicData].
`HELPrct` contains data from
...
that the most interesting parts of statistics involve more than one variable [@Wild:RSS:2011],
and 
...
displays on the first day of class [@Wang:USCOTS:2015].
...
[@Tintle:TAS:2015; Hesterberg:2015].
...
[@Salsburg:2002] for the details of this famous story.)  But here we will
...
[@Hesterberg:2015].  The simplest method is to use quantiles 
...
been added to \pkg{mosaic} to facilitate extracting information from more
...
\pkg{lattice} or \pkg{ggplot2} [@ggplot2] graphics instead of base graphics, 
...
depends on \pkg{dplyr} [@dplyr], but for a different reason.  The functions in \pkg{dplyr} 
...
See [@Baumer:RMarkdown:2014] for a discussion of how R Markdown can be used 
...
Packages such as \pkg{resample} [@resample] 
...
undercover, and miss more often in one direction than in the other [@Hesterberg:2015].

## Efficiency Issues

For applications where speed is of utmost 
importance, it is better to avoid some of the \pkg{mosaic} wrappers. 
For example, for the numerical summary functions, the \pkg{mosaic} versions cannot be
...
```
\noindent
Furthermore, `do()` can take advantages of multiple cores 
if the \pkg{parallel} package is attached.
Even on a laptop with a single quad-core processor, the speed-up is noticible.
```{r, parallel-benchmark}
library(parallel)
