...
of data sets and set the number of digits to display by default.\footnote{If the \pkg{parallel} package is installed, loading it will allow the \function{do} function
...
@
Now conduct 1000 resampling trials\footnote{1000 is a resonable number of trials 
...
makes it clear which data set an variable are beging considered and that we are 
...
Of course, \R{} can hanlde all of these little details and do the entire 
...
confidence intervals based on percentiles or bootstrap standard errors may not
