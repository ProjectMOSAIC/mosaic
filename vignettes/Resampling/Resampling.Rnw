...
mean(~Price, data=mustangs)
@
we are anticipating the next steps: the introduction of 
...
<<hist,fig.width=4, fig.height=3>>=
histogram(~ result, data=trials, xlab="Mean Mustang Price (in thousand dollars)")
@ 
...
@
<<flips,fig.width=4, fig.height=3>>=
...
<<>>=
trials <- do(1000) * cor(Price, Miles, data=resample(mustangs))
quantiles <- qdata(c(.025, .975), trials$result); quantiles
@

<<cor,fig.width=4, fig.height=3>>=
...
<<sleep,tidy=FALSE,fig.width=4, fig.height=3>>=
