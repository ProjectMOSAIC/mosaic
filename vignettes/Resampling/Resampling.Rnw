...
Those familiar with \R{} may wonder why the statement has not been written as \texttt{mean(mustangs\$Price)}.  You can, of course, carry out exactly the same calculation this way.  By using \verb+mean(~Price, data=mustangs)+, we are anticipating the next steps: the introduction of 
