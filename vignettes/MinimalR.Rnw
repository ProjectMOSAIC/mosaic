...
%\VignetteIndexEntry{Minimal R (lattice version)}
...
\chead{\sf \bfseries \Large Enough R for Intro Stats (lattice version)}
\rhead{July, 2017}
\lhead{R. Pruim}
...
\begin{enumerate}
	...
	\item 
		...
		Recently, we have introduced another package, \pkg{ggformula}, that
		provides a similar formula interface to \pkg{ggplot2}. We anticipate
		that this will be our graphics solution going forward.  A separate vignette
		demonstrates how to use \pkg{ggformula} in place of \pkg{lattice}. 
\end{enumerate}
...
\rhead{\href{http://mosaic-web.org}{http://mosaic-web.org} (\Sexpr{Sys.Date()})}
