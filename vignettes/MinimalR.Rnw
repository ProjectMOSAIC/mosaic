...
The \code{goal} is whatever
function is needed to have R do what you want.
...
\vspace*{-.20in}
<<>>=
do(2) * rflip(20, prob=0.2)
coins <- do(1000)* rflip(6, prob=0.5)
...
%\columnbreak
<<tally>>=
require(mosaicData)   # package data sets used below
